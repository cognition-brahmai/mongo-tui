"""Small non-secret Mongrove application settings store."""
...
class SettingsStore:
    ...
    def __init__(self, path: Path | None = None) -> None:
        self.path = path or user_config_path("mongrove") / "settings.json"
    ...
    def _load(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise SettingsStoreError(f"Could not read Mongrove settings: {error}") from error
        if not isinstance(value, dict):
            raise SettingsStoreError("Mongrove settings must contain a JSON object.")
        return value
    ...
    def _write(self, settings: dict[str, Any]) -> None:
        try:
            ...
            raise SettingsStoreError(f"Could not save Mongrove settings: {error}") from error
