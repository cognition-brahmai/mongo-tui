...
from mongotui.domain.namespace import CollectionInfo, NamespaceTreeItem
from mongotui.domain.query import FindQuery, QueryFormState, QueryValidationError, parse_find_query
from mongotui.services.bson_codec import format_cell
from mongotui.services.mongo_gateway import DocumentsPage, MongoGatewayError
from mongotui.ui.screens.document import DocumentScreen
from mongotui.ui.screens.query_options import QueryOptionsScreen
...
class BrowserScreen(Screen[None]):
    ...
    def on_mount(self) -> None:
        ...
        tree.root.expand()
        connection_info = self.mongo_app.connection_info
        if connection_info is not None:
            ...
            mode = "READ ONLY" if self.mongo_app.read_only else "READ MODE"
    ...
    def action_disconnect(self) -> None:
        self.mongo_app.gateway.disconnect()
        self.mongo_app.connection_info = None
        self.app.pop_screen()

    @work(thread=True, exclusive=True, group="databases", exit_on_error=False)
    def _load_databases(self) -> None:
        try:
            databases = self.mongo_app.gateway.list_databases()
    ...
    def _load_collections(self, database: str) -> None:
        try:
            collections = self.mongo_app.gateway.list_collections(database)
        except MongoGatewayError as error:
    ...
    def _load_documents(
        ...
        try:
            result = self.mongo_app.gateway.fetch_documents(
                database,
                collection,
                query,
                page=page,
    ...
    def _render_databases(self, databases: list[str]) -> None:
        ...
        startup_database = self.mongo_app.startup_database
    ...
    def _render_collections(self, database: str, collections: list[CollectionInfo]) -> None:
        ...
        startup_database = self.mongo_app.startup_database
        startup_collection = self.mongo_app.startup_collection
