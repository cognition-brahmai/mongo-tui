"""The MongoTUI Textual application shell."""
...
from mongotui.services.mongo_gateway import MongoGateway, PyMongoGateway
from mongotui.services.profile_store import ProfileStore
from mongotui.services.settings_store import SettingsStore, SettingsStoreError
from mongotui.ui.themes import (
    CURATED_THEME_NAMES,
    CUSTOM_THEMES,
    DEFAULT_THEME,
    get_theme_option,
...
class MongoTUIApp(App[None]):
    ...
    def select_theme(self, name: str, *, persist: bool = True) -> None:
        ...
        if name not in CURATED_THEME_NAMES:
            raise ValueError(f"Unsupported MongoTUI theme: {name}")
        self.theme = name
        if persist:
