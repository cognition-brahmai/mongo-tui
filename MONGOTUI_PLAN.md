# MongoTUI Product and Technical Plan

## 1. Purpose

MongoTUI is a Python terminal user interface for working directly with MongoDB deployments on servers where a graphical MongoDB Compass client is unavailable or unsuitable.
...
MongoTUI will be a keyboard-first application built with Textual and PyMongo. Mouse input enhances the experience when terminal mouse reporting is available, but no function may depend on a mouse.
...
```text
+ MongoTUI -- Production EU [PRIMARY] ------------------------------------+
...
```text
~/.config/mongotui/config.json
~/.config/mongotui/connections.json
~/.local/share/mongotui/history.sqlite3
~/.cache/mongotui/
```
...
```text
mongotui
mongotui mongodb://localhost:27017
mongotui --profile production
mongotui --database app --collection customers
mongotui --read-only
mongotui --no-history
mongotui --theme monochrome
python -m mongotui
```
...
- Installable with `pipx install mongotui`.
- Launchable using `mongotui` and `python -m mongotui`.
